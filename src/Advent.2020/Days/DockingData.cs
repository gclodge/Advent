...
public class BitValue
{
    ...
    public bool[] Bits { get; } = new bool[NumBits];
    ...
    public BitValue(string[] line, bool v2 = false)
    {
        ...
        for (int i = 0; i < NumBits; i++) Bits[i] = arr[StartIndex - i];
    }

    static ulong ParseIndex(string s)
    {
        var val = s[4..].Replace("]", "");
...
public class BitMask
{
    ...
    public HashSet<byte> Wildcards { get; } = new();
    public Dictionary<byte, bool> Mask { get; } = new();
...
public class Docker
{
    ...
    {
        if (Mask == null) throw new NullReferenceException($"Mask is set to null - fix it!");
